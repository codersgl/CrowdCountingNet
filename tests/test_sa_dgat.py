...
class TestCopyPasteAugmentation:
    def test_import(self):
        from crowdcount.data.transforms import CopyPasteCrowdAugmentation

        aug = CopyPasteCrowdAugmentation(paste_prob=1.0, num_pastes=1)
    ...
    def test_shapes_preserved(self):
        ...
        aug = CopyPasteCrowdAugmentation(paste_prob=1.0, num_pastes=2)
        image = np.random.randint(0, 255, (128, 128, 3), dtype=np.uint8)
        points = np.random.rand(20, 2) * 128
        density = np.random.rand(128, 128).astype(np.float32)

        out_img, out_pts = aug(image, points, density)
        assert out_img.shape == image.shape
        assert out_pts.shape[1] == 2
        # Points should be at least as many as original (paste adds points)
        assert len(out_pts) >= len(points)

    def test_no_op_on_zero_prob(self):
        import numpy as np
        from crowdcount.data.transforms import CopyPasteCrowdAugmentation

        aug = CopyPasteCrowdAugmentation(paste_prob=0.0)
        image = np.random.randint(0, 255, (64, 64, 3), dtype=np.uint8)
        points = np.random.rand(10, 2) * 64

        out_img, out_pts = aug(image, points)
        assert len(out_pts) == len(points)
