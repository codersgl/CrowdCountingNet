...
def build_model(cfg: DictConfig, training: bool = False):
    ...
    model = DSGCnet(
        ...
        gcn_feature_conv_type=getattr(cfg.model, "gcn_feature_conv_type", "gcn"),
        gcn_deformable_heads=int(getattr(cfg.model, "gcn_deformable_heads", 4)),
        gcn_deformable_lambda=float(getattr(cfg.model, "gcn_deformable_lambda", 1.0)),
        gcn_deformable_mu=float(getattr(cfg.model, "gcn_deformable_mu", 1.0)),
        gcn_deformable_dropout=float(getattr(cfg.model, "gcn_deformable_dropout", 0.1)),
        unified_deform_attn_cfg=getattr(cfg.model, "unified_deform_attn", None),
